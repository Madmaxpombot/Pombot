...
# ==========================================
ADMIN_USER = os.getenv("ADMIN_USER", "madmax")
DEFAULT_PASS = os.getenv("ADMIN_PASS", "madmax@123")
AUTH_COOKIE_NAME = "session_token"
AUTH_SECRET = "admin_authenticated_session_key_99"
DB_NAME = "madmax_database.db"
...
async def init_db():
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        # Migration check: add access_link column if not present in existing db
        try:
        ...
        defaults = {
            ...
            "admin_chat_id": "8854673229",
            "maintenance": "off",
            "upi_id": "BHARATPE2I0V0R6N3P47486@unitype",
            "payee_name": "MISS NASEERA",
        ...
        default_plans = [
            ("plan_1", "INDIAN WEBSERIES", 99.0, "30 Days", "https://t.me/+exampleLink1"),
            ("plan_2", "3 MONTHS SPECIAL", 249.0, "90 Days", "https://t.me/+exampleLink2"),
            ("plan_3", "6 MONTHS VIP", 449.0, "180 Days", "https://t.me/+exampleLink3"),
            ("plan_4", "1 YEAR ACCESS", 799.0, "365 Days", "https://t.me/+exampleLink4"),
            ("plan_5", "LIFETIME PASS", 1299.0, "Lifetime", "https://t.me/+exampleLink5"),
...
async def get_admin_menu():
    ...
    builder.button(text="Upload / Edit Demos", callback_data="adm_edit_video")
...
# Helper function to deliver approval with optional premium link
...
async def nav_my_premium(callback: types.CallbackQuery):
    ...
    # Fetch access link if user has premium plan
    builder = InlineKeyboardBuilder()
    if plan != "Free":
...
async def start_edit_demo_video(callback: types.CallbackQuery, state: FSMContext):
    ...
    if callback.from_user.id in admin_ids:
        ...
        await callback.message.edit_text(
            "📹 <b>Send Demo Video or URL</b>:\n\n"
            "• You can send a direct video clip right here!\n"
...
async def process_new_demo_video(message: types.Message, state: FSMContext):
    ...
    # Check if admin sent a real video directly in Telegram
    if message.video:
        file_id = message.video.file_id
        curr_videos = await get_demo_video_list()
        curr_videos.append(file_id)
        await update_setting("demo_video", "\n".join(curr_videos))
        await state.clear()
        await message.answer(f"✅ Video uploaded to demo playlist! Total videos: {len(curr_videos)}", reply_markup=await get_admin_menu())
        return

    if message.text:
        await update_setting("demo_video", message.text.strip())
        await state.clear()
        await message.answer("✅ Demo playlist updated!", reply_markup=await get_admin_menu())
        return

    await message.answer("Please send a video file or text URL.")
...
async def process_edit_plan_validity(message: types.Message, state: FSMContext):
    if message.text == "/cancel":
        ...
        await message.answer("Canceled.", reply_markup=await get_admin_menu())
        return
    await state.update_data(new_pvalidity=message.text.strip())
    await state.set_state(AdminStates.waiting_for_plan_link)
    await message.answer("Step 4: Enter Optional Premium Invite Link (or send 'none' to leave blank):")


@manager.dp.message(AdminStates.waiting_for_plan_link)
async def process_edit_plan_link(message: types.Message, state: FSMContext):
    if message.text == "/cancel":
        await state.clear()
        await message.answer("Canceled.", reply_markup=await get_admin_menu())
        return
    link = "" if message.text.strip().lower() in ["none", "no", "skip", "-"] else message.text.strip()
    d = await state.get_data()
    await update_plan(d["target_pid"], d["new_pname"], d["new_pprice"], d["new_pvalidity"], link)
    await state.clear()
    await message.answer("Plan updated with access link!", reply_markup=await get_admin_menu())
    ...
    if callback.from_user.id in admin_ids:
        ...
        async with aiosqlite.connect(DB_NAME) as db:
            ...
            if act == "approved":
                ...
                # Deliver notification and optional link
...
<body class="text-slate-100 min-h-screen flex overflow-x-hidden relative">
        ...
        <div class="p-4 md:p-8 max-w-5xl w-full mx-auto space-y-6">
            ...
            <form method="POST" action="/admin/save">
                ...
                <div id="tab-media" class="tab-content space-y-5 hidden">
                    <div class="glass-card rounded-2xl p-6 space-y-5">
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-purple-300 uppercase tracking-wider mb-2 font-mono">Demo Videos Playlist (1 URL or Telegram file_id per line)</label>
                            <textarea name="demo_video" rows="4" required placeholder="https://.../video1.mp4&#10;https://.../video2.mp4"
                                      class="w-full bg-[#070410]/90 border border-purple-900/60 rounded-xl px-4 py-2.5 text-sm text-white font-mono focus:outline-none focus:border-cyan-400 transition">{{ demo_video }}</textarea>
                        </div>
                    </div>

                    <!-- Direct Video File Upload to Dashboard -->
                    <div class="glass-card rounded-2xl p-6 space-y-4">
                        <h4 class="font-tech text-sm font-bold text-cyan-300 tracking-wide">Upload New Demo Video File Directly</h4>
                        <div class="flex flex-col sm:flex-row items-center gap-3">
                            <input type="file" id="demo_file_input" accept="video/mp4,video/*"
                                   class="w-full bg-[#070410]/90 border border-purple-900/60 rounded-xl px-4 py-2 text-xs text-purple-300 file:mr-4 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-cyan-500/20 file:text-cyan-300 hover:file:bg-cyan-500/30">
                            <button type="button" onclick="uploadDemoVideoFile()" class="w-full sm:w-auto bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white font-tech text-xs px-5 py-2.5 rounded-xl uppercase tracking-wider transition whitespace-nowrap">
                                📤 Upload Video
                            </button>
                        </div>
                        <p id="uploadStatus" class="text-xs text-purple-400 font-mono"></p>
                    </div>
                </div>
            ...
            <div id="tab-plans" class="tab-content space-y-5 hidden">
                <div class="glass-card rounded-2xl p-6 space-y-4">
                    ...
                    <form method="POST" action="/admin/plans/add" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3 items-end font-mono text-xs">
                        <div>
                        ...
                        <div>
                            <label class="block text-[11px] font-semibold text-purple-300 mb-1.5 uppercase">Invite / Access Link</label>
                            <input type="text" name="access_link" placeholder="https://t.me/+..." class="w-full bg-[#070410]/90 border border-purple-900/60 rounded-xl px-3 py-2 text-white focus:outline-none focus:border-cyan-400">
                        </div>
                        <div>
                            ...
                            <tbody class="divide-y divide-purple-900/30">
                                ...
                                <tr class="hover:bg-purple-900/20 transition">
                                    <form method="POST" action="/admin/plans/update">
                                        ...
                                        <td class="p-3"><input type="text" name="access_link" value="{{ access_link }}" placeholder="https://t.me/+..." class="bg-[#070410]/90 border border-purple-900/60 rounded-lg px-2 py-1 text-white w-36"></td>
    ...
    <script>
        ...
        async function uploadDemoVideoFile() {
            const input = document.getElementById('demo_file_input');
            const statusEl = document.getElementById('uploadStatus');
            if (!input.files || input.files.length === 0) {
                statusEl.innerText = "Please select a video file first.";
                statusEl.className = "text-xs text-rose-400 font-mono";
                return;
            }

            statusEl.innerText = "Uploading video to server...";
            ...
            try {
                const res = await fetch('/admin/upload-demo-video', {
                    method: 'POST',
                    body: formData
                });
                const data = await res.json();
                if (data.status === 'success') {
                    statusEl.innerText = "Video successfully uploaded and added to playlist!";
                    statusEl.className = "text-xs text-emerald-400 font-mono";
                    // Append URL to textarea
                    const txtArea = document.querySelector('textarea[name="demo_video"]');
                ...
                statusEl.innerText = "Upload error occurred: " + e;
                statusEl.className = "text-xs text-rose-400 font-mono";
            }
        }
...
async def upload_demo_video_file(
    ...
    try:
        ...
        # Build accessible server URL
        base_url = str(request.base_url).rstrip("/")
        file_url = f"{base_url}/static/uploads/{clean_name}"

        # Automatically append to demo list

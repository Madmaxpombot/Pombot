...
async def get_admin_menu():
    ...
    builder.button(text="Edit Demo Videos", callback_data="adm_edit_video")
...
async def nav_demo(callback: types.CallbackQuery):
    ...
    # Parse index from callback: btn_view_demo or btn_view_demo:X
...
async def nav_my_premium(callback: types.CallbackQuery):
    ...
    msg = await manager.bot.send_message(
        chat_id=callback.message.chat.id,
        text=text,
        reply_markup=get_demo_keyboard(0, 1),
    )
    track(callback.message.chat.id, msg.message_id)
...
async def start_edit_demo_video(callback: types.CallbackQuery, state: FSMContext):
    ...
    if callback.from_user.id in admin_ids:
        ...
        await callback.message.edit_text("Send video URLs or file IDs (one per line):\n/cancel to abort.")
        await callback.answer()
            ...
            <form method="POST" action="/admin/save">
                ...
                <div id="tab-media" class="tab-content space-y-5 hidden">
                    <div class="glass-card rounded-2xl p-6 space-y-5">
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-purple-300 uppercase tracking-wider mb-2 font-mono">Demo Videos Playlist (1 URL per line)</label>
                            ...
                            <p class="text-[11px] text-purple-400/80 mt-1 font-mono">Add as many URLs as you like. The bot will generate Next/Previous buttons automatically.</p>
                        </div>
                    </div>
                </div>

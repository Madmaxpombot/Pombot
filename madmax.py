...
<body class="text-slate-100 min-h-screen flex overflow-x-hidden relative">
    ...
    <script>
        ...
        // 2. Safe Countdown Timer
        try {
            const rawExpiry = "{{ panel_expiry }}";
            const cleanExpiry = rawExpiry ? rawExpiry.replace(' ', 'T') : "";
            const expiryDate = cleanExpiry ? new Date(cleanExpiry).getTime() : NaN;
